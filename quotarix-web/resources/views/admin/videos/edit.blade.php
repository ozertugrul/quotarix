...
<div class="mb-4">
    ...
    <h2 class="fw-bold text-navy mt-1">Video Düzenle: {{ $video->title }}</h2>
</div>
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm p-4 mb-4" style="border-radius: 20px; background: #fff;">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="title" class="form-control" value="{{ old('title', $video->title) }}" required style="border-radius: 10px; padding: 11px 14px;">
                </div>

                <div class="mb-3">
                    <label class="form-label fw-bold small text-navy">Video URL (YouTube veya Vimeo) <span class="text-danger">*</span></label>
                    <input type="url" name="video_url" id="videoUrlInput" class="form-control font-monospace" value="{{ old('video_url', $video->video_url) }}" required style="border-radius: 10px; padding: 11px 14px;">
                </div>
            ...
            <div class="card border-0 shadow-sm p-4 mb-4" style="border-radius: 20px; background: #fff;">
                ...
                <div class="mb-3">
                    ...
                    <select name="placement" class="form-select" style="border-radius: 10px; padding: 11px 14px;">
                        <option value="home" {{ old('placement', $video->placement) == 'home' ? 'selected' : '' }}>Ana Sayfa (#video bölümü)</option>
                        <option value="features" {{ old('placement', $video->placement) == 'features' ? 'selected' : '' }}>Özellikler Sayfası</option>
                        <option value="why" {{ old('placement', $video->placement) == 'why' ? 'selected' : '' }}>Neden Quotarix Sayfası</option>
                    </select>
                </div>

                <div class="mb-3">
                    <label class="form-label fw-bold small text-navy">Özel Kapak Görseli</label>
                    @if($video->thumbnail)
                        <div class="mb-2">
                            <img src="{{ asset($video->thumbnail) }}" alt="Thumbnail" class="img-thumbnail" style="max-height: 80px;">
                        </div>
                    @endif
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="sort_order" class="form-control" value="{{ old('sort_order', $video->sort_order) }}" style="border-radius: 10px; padding: 11px 14px;">
                </div>

                <div class="form-check form-switch mb-4 fs-5">
                    <input class="form-check-input" type="checkbox" name="is_active" id="isActive" value="1" {{ old('is_active', $video->is_active) ? 'checked' : '' }}>
