...
<div class="mb-4">
    ...
    <h2 class="fw-bold text-navy mt-1">Plan Düzenle: {{ $plan->name }}</h2>
</div>
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm p-4 mb-4" style="border-radius: 20px; background: #fff;">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" class="form-control" value="{{ old('name', $plan->name) }}" required style="border-radius: 10px; padding: 11px 14px;">
                </div>

                <div class="row g-3 mb-3">
                    <div class="col-md-4">
                        <label class="form-label fw-bold small text-navy">Fiyat (Özel teklif ise boş bırakın)</label>
                        <input type="number" step="0.01" name="price" class="form-control" value="{{ old('price', $plan->price) }}" style="border-radius: 10px; padding: 11px 14px;">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-bold small text-navy">Para Birimi</label>
                        <input type="text" name="currency" class="form-control" value="{{ old('currency', $plan->currency) }}" style="border-radius: 10px; padding: 11px 14px;">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-bold small text-navy">Fatura Dönemi</label>
                        <input type="text" name="period" class="form-control" value="{{ old('period', $plan->period) }}" style="border-radius: 10px; padding: 11px 14px;">
                    </div>
                </div>

                @php
                    $rawFeatures = is_array($plan->features_list) ? implode(PHP_EOL, $plan->features_list) : '';
                @endphp

                <div class="mb-3">
                    <label class="form-label fw-bold small text-navy">Özellikler (Her satıra bir özellik yazınız)</label>
                    <textarea name="features_raw" class="form-control" rows="8" style="border-radius: 10px; padding: 11px 14px;">{{ old('features_raw', $rawFeatures) }}</textarea>
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm p-4 mb-4" style="border-radius: 20px; background: #fff;">
                ...
                <div class="form-check form-switch mb-3 fs-5">
                    <input class="form-check-input" type="checkbox" name="is_featured" id="isFeatured" value="1" {{ old('is_featured', $plan->is_featured) ? 'checked' : '' }}>
                ...
                <div class="form-check form-switch mb-4 fs-5">
                    <input class="form-check-input" type="checkbox" name="is_active" id="isActive" value="1" {{ old('is_active', $plan->is_active) ? 'checked' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="sort_order" class="form-control" value="{{ old('sort_order', $plan->sort_order) }}" style="border-radius: 10px; padding: 11px 14px;">
                </div>
